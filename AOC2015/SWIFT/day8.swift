...
"""###
  ...
  func decode(_ str: String) -> String {
    var working = String(str.dropFirst().dropLast())
    func get() -> Character? {
    ...
    while let char = get() {
      if char == "\\" {
        guard let char2 = get() else { fatalError("Escape without following character!") }
        switch char2 {
        ...
        case "x":
          guard let _ = get() else { fatalError("Escape x without enough chars") }
          guard let _ = get() else { fatalError("Escape x without enough chars") }
          result.append("x")
        default:
          fatalError("Invalid escape!")
        }
      } else {
        result.append(char)
      }
  ...
  for line in input {
    sumLine += line.count
    sumDecoded += decode(line).count
    sumExtra += (line.map{$0}.filter{$0 == "\\"}.count)  +
    (line.map{$0}.filter{$0 == "\""}.count) 
    + 2
  }
  
  print("Part a: \(sumLine - sumDecoded)")
  print("Part b: \(sumExtra)")
