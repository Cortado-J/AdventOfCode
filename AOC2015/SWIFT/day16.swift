...
"""
  ...
  let itemParser = Prefix { $0 != ":" }
    .skip(": ")
    .take(Int.parser())
    .map{ Item(name: String($0.0), count: $0.1) }
  let itemsParser = Many(itemParser, separator: "\n")
  let items = itemsParser.parse(itemList)!
  let itemDict = Dictionary(uniqueKeysWithValues: items.map{ ($0.name, $0.count) } )
  ...
  // Sample to parse: "Sue 500: cats: 2, goldfish: 9, children: 8"
  let sueParser =
  Skip("Sue ")
    .take(Int.parser())
    .skip(": ")
    ...
    .skip(", ")
    .take(itemParser)
    .map{ Sue(index: $0.0, items: [$0.1, $0.2, $0.3]) }

  let suesParser = Many(sueParser, separator: "\n")
  
  let sues = suesParser.parse(input)!
  //print(sues)
    
  for sue in sues {
    func checka(_ index: Int) -> Bool {
      itemDict[sue.items[index].name]! == sue.items[index].count
    }
    if checka(0) && checka(1) && checka(2) { print("Part a: \(sue.index)") }
    func checkb(_ index: Int) -> Bool {
      let numberInList = itemDict[sue.items[index].name]!
      let numberInReport = sue.items[index].count
      switch sue.items[index].name {
      case "cats", "trees": return numberInList < numberInReport
      case "pomeranians", "goldfish": return numberInList > numberInReport
      default: return numberInList == numberInReport
      }
    }
    if checkb(0) && checkb(1) && checkb(2) { print("Part b: \(sue.index)") }
  }
}
