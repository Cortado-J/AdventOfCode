...
"""
  ...
  func on(_ x: Int, _ y: Int) {
    if _part == "a" { lights[x][y] = 1 }
    else { lights[x][y] += 1 }
  }
  func off(_ x: Int, _ y: Int) {
    if _part == "a" { lights[x][y] = 0 }
    else { lights[x][y] = max(0, lights[x][y] - 1) }
  }

  func toggle(_ x: Int, _ y: Int) {
    if _part == "a" { lights[x][y] = 1-lights[x][y] }
    else { lights[x][y] += 2 }
  }

  func change(from: String.SubSequence, to: String.SubSequence, changeFunc: (Int,Int) -> () ) {
    let fromxy = String(from).split(separator: ",").map{Int($0)!}
    let fromx = fromxy[0]
    let fromy = fromxy[1]

    let toxy   = String(to  ).split(separator: ",").map{Int($0)!}
    let tox = toxy[0]
    let toy = toxy[1]

    for x in stride(from: fromx, through: tox, by: fromx > tox ? -1 : +1) {
      for y in stride(from: fromy, through: toy, by: fromy > toy ? -1 : +1) {
        changeFunc(x,y)
      }
    }
  }
  
  func solve(part: String) -> Int {
    _part = part
    reset()
    for line in input {
      if line[0] == "turn" {
        change(from: line[2], to: line[4], changeFunc: line[1] == "on" ? on : off)
      } else {
        if line[0] == "toggle" {
          change(from: line[1], to: line[3], changeFunc: toggle)
        }
      }
    }
  ...
  print("Part a: \(solve(part: "a"))")
  print("Part b: \(solve(part: "b"))")
}
