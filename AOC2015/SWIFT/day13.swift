...
"""
  ...
  for move in input {
    let person = String(move[0])
    persons.insert(person)
    let neighbour = String(move[10].dropLast())
    let changeValue = Int(move[3])! * (move[2] == "gain" ? 1 : -1)
    moves[(person + ">" + neighbour)] = changeValue
  }
  //print(moves)
  
  var guests = Array(persons)
  ...
  func maxScore() -> Int {
    var result = Int.min
    for perm in guests.permutations() {
      let toLeft = Array(perm.rotatedLeft())
      let toRight = Array(perm.rotatedRight())
      var score = 0
      for index in perm.indices {
        score += moves[ perm[index] + ">" + toRight[index] ]!
        score += moves[ perm[index] + ">" +  toLeft[index] ]!
      }
