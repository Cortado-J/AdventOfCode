...
"""
    ...
    .map{ $0.split(separator: "|") } // Split by the |
  
  let parta = input
    .map{ $0[1] } // Choose just right hand of each pair (i.e. after the | )
    .map{
      $0.trimmingCharacters(in: .whitespaces) // Trim whitespace
        .split(separator: " ") // Seperate by spaces
        .map{ String($0).count } // Turn into integers
    }
    .reduce([], +)  // Flatten
    .filter{ [2,3,4,7].contains($0)} // Only digits 1,4,7,8 which have 2,4,3,7 segments
    .count // How many are there?

  print("Day8")
  print(parta)
  
  typealias Mapping = [String]

  func mapped(char: Character, mapping: Mapping) -> Character {
    mapping[Int(char.asciiValue!) - Int(Character("a").asciiValue!)].first!
  }

  func mapped(segment: String, mapping: Mapping) -> String {
    segment
      .map{ String(mapped(char: $0, mapping: mapping)) }
      .sorted()  // Sorted
      .joined()  // Join back into string
  }
  
  func digit(_ segment: String) -> Int? {
    ["abcefg",  // 0
     "cf",      // 1
     "acdeg",   // 2
     "acdfg",   // 3
     ...
     "abcdefg", // 8
     "abcdfg"]  // 9
      .firstIndex(of: segment)
  }

  // Find a map which works for all segments
  func findMapFor(segments: [String]) -> Mapping? {
  outerLoop:
    for mapping in ["a","b","c","d","e","f","g"].permutations() {
      for segment in segments {
        let value = mapped(segment: segment, mapping: mapping)
        if digit(value) == nil { continue outerLoop }
      }
      // The mapping has worked for all segments!!!!
      return mapping
    }
    fatalError("No mapping found for: \(segments)")
  }
  ...
  for row in partb {
    ...
    let right = row[1]
      ...
      .split(separator: " ") // Seperate by spaces
      .map{String($0)}
    let mapping = findMapFor(segments: Array(right) + Array(left))!
    let rightMapped = right.map{
      mapped(segment: $0, mapping: mapping)
    }
    let digits = Int(
      rightMapped.map{
        String(digit($0)!)
    ...
    print(left, right, mapping, rightMapped, digits)
    resultB += digits
  }
  print(resultB)

  //  print(findMapFor(segments: ["agf"])!)
