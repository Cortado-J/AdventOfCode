...
indirect enum VacuumProgram {
  case turn(turn: Turn, steps: Int)
  case nested(program: [VacuumProgram])
  
  /// Unpack nested programs
  func unpacked() -> [VacuumProgram] {
    switch self {
    case .nested(let program): return program.flatMap{ $0.unpacked() }
    default:                   return [self]
    }
  }
  
  func steps(startingDirection: Direction) -> [Direction] {
    var result: [Direction] = []
    var currentDirection = startingDirection
    for step in unpacked() {
      /// We know that steps are either left or right:
      switch step {
      case .turn(let turn, let steps):
        currentDirection = startingDirection.turned(turn)
        result += Array(repeating: currentDirection, count: steps)
      case .nested:
        fatalError("nested should be stripped by unpacked!")
      }
    }
    return result
  }
...
struct Ascii {
  ...
  init() {
    viewer = IntCode(program: program)
  }
  
  mutating func step(input: [Int] = []) -> [Character] {
    viewer
      .run(input: input)
      .map{ Character(UnicodeScalar($0)!) }  /// Turn into characters
  }
  
  mutating func intersections() -> [(Int,Int)] {
    let newline = Character(UnicodeScalar(10))
    let data = step()
    let width = data.firstIndex(of: newline )!
    let array = data
      .filter{ $0 != newline }  /// Strip newlines
      .chunked(into: width)  /// Chunk into rows so we have [[Character]]
    var intersects: [(Int,Int)] = []
    for row in (1...array.count-2) { /// Don't bother with first and last row
      for col in (1...width-2) {  /// Don't bother with first and last column
        
        func hash(_ drow: Int, _ dcol: Int) -> Bool {
          array[row + drow][col + dcol] == Character("#")
        }
        
        if hash(0,0) && hash(0,1) && hash(0,-1) && hash(1,0) && hash(-1,0) {
          intersects.append( (row,col) )
        }
      }
    }
    return intersects
  ...
  func checkReturn(_ message: String, expect: String) {
    guard message == expect else { fatalError("Expected '\(expect)' but received: '\(message)'")}
    print("\(message)")
  }

  /// DropFirst allows discarding some chars
  /// DropLast allows discarding some chars from end and defautls to 1 (newline)
  mutating func sendAndGet(_ send: String = "", droppingFirst: Int = 0, droppingLast: Int = 1, withNewline: Bool = true) -> String {
    let newline = String(Character(UnicodeScalar(10)))
    return step(input: (send + (withNewline && send.count > 0 ? newline : ""))
  ...
  mutating func explore() {
    viewer = IntCode(program: program) /// Ensure reset
    viewer.memory.patch(address: 0, value: 2) /// Wakeup robot
    print("=====================")
    checkReturn(sendAndGet(droppingFirst: 1255, droppingLast: 1), expect: "Main:")
    checkReturn(sendAndGet("A,C,A,C,B,B,C,A,C,B"), expect: "Function A:")
    checkReturn(sendAndGet("L,4,R,8,L,6,L,10"), expect: "Function B:")
    checkReturn(sendAndGet("L,4,L,4,L,10"), expect: "Function C:")
    checkReturn(sendAndGet("L,6,R,8,R,10,L,6,L,6"), expect: "Continuous video feed?")
    let result = viewer.run(input: [Int(Character("n").asciiValue!), 10] )
    print(result)
    print("=====================")
  }
  ...
  // Which manually translates to:
  //  A: L,4,R,8,L,6,L,10,L,6,R,8,R,10,L,6,L,6,
  //  A: L,4,R,8,L,6,L,10,L,6,R,8,R,10,L,6,L,6,
  //  B: L,4,L,4,L,10,
  //  B: L,4,L,4,L,10,
  //  C: L,6,R,8,R,10,L,6,L,6,
