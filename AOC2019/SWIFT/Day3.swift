...
//  AdventOfCode2019
//
//  Created by Adahus on 03/12/2019.
...
struct Day3 {
  ...
  func partAandB() -> (Int,Int) {
    ...
    func getFirstVisits(path: [Point], intersects: Set<Point>) -> [(Point, Int)] {
      let numberedIntersectOnPath = numberedIntersectsOnPath(path: path, select: intersects)
      /// We have numberedIntersectOnPath as [(index, Point)]
      /// But there may be repeated points so we need to choose the ones with the least index
      /// First an array of the unique points:
      let uniquePoints = Array(Set(
        numberedIntersectOnPath
          .map{ $0.1 }
      ))
      
      /// Then a list of points with the minimum indices:
      return uniquePoints
        .map{ point in
          (
            point,
            numberedIntersectOnPath
              /// Select just those that match point: [Point: [index]]
              .filter{ (_, innerPoint) in
                innerPoint == point }
              /// And find the one with the minimum index: [Point: index]
              .reduce(Int.max) {
                minimum, indexPoint in min(minimum, indexPoint.0)
            }
          )
    ...
    let firstVisitsOnPath1 = getFirstVisits(path: path1, intersects: intersects)
    let firstVisitsOnPath2 = getFirstVisits(path: path2, intersects: intersects)
    /// firstVisits are: [(Point, Int)]
    /// We need the combined path lengths: [Point: length] so:
    let partB = 2+firstVisitsOnPath1
      .map{ (arg) -> (Point, Int) in
        let (point1, index1) = arg
        return (point1,
                index1
                  +
                  firstVisitsOnPath2
                    .first{ (point2, index2) in point1 == point2 }! /// Lookup matching point
                    .1  /// And choose corresponding index
        ) /// We now have [Point, length]
    }
    .map{ $0.1 } /// Now just a list of lengths
    .reduce(Int.max) { min($0, $1) }  /// And now the minimum length

    return (partA,partB)
  }

//  func partAandB() -> (a: Int, b: Int) {
//    let path1 = path(pathSpec1) // Length = 154530
//    let path1Set = Set(path1)
//    let path2 = path(pathSpec2) // Length = 157570
//    for point1 in path1 {
//      print("Point1: \(point1)")
//      for point2 in path2 {
//        if point1 == point2 {
//          print("Intersection: \(point1), \(point2)")
//        }
//      }
//    }
//
//      .intersection(Set(path(pathSpec2)))
//    print(intersections)
//    let result = intersections
//      .map{ $0.manhattan }
//      .reduce(100000) { $1 < $0 ? $1 : $0 }
//    return 1
//  }
}
