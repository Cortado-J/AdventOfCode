...
func process() {
  print("Adding: \(gather) -> \(gather.count)")
  count += gather.count
  reset()
}
...
for line in lines {
  ...
  var traveller: Set<Character> = []
  for char in line {
    traveller.insert(char)
  }
  if traveller.count > 0 {
    print("Gather:\(gather) Traveller:\(traveller)")
    gather.formIntersection(traveller)
    print("Gather:\(gather) Traveller:\(traveller)")
  }
}
process()
