...
struct Grid: Equatable {
  ...
  func cell(row: Int, col: Int) -> Character {
    //print("X")
    guard row >= 0 && row <= cells.count-1
      && col >= 0 && col <= cells[0].count-1 else {
        return "E" //for Edge
    }
    //print("Y")
  ...
  func neighbours(row: Int, col: Int) -> Int {
    var count = 0
    for rowOffset in -1...1 {
      for colOffset in -1...1 {
        //print("a",row,col)
        if rowOffset != 0 || colOffset != 0 {
          if cell(row: row+rowOffset, col: col+colOffset) == "#" {
            count += 1
          }
        }
      }
  ...
  func neighboursLong(row: Int, col: Int) -> Int {
    var count = 0
    for rowOffset in -1...1 {
      for colOffset in -1...1 {
        //print("a",row,col)
        if rowOffset != 0 || colOffset != 0 {
          ...
          loop: while true {
            ...
            switch cell(row: row+totalRowOffset, col: col+totalColOffset) {
            case "#": count += 1; break loop // Found a neighbour so count it and stop looping
            case ".": break //Keep looping to look for the next seat in that direction
            case "L": break loop // Found an empty seat so don;t count and stop looking
            default: break loop // "E" for Edge or anything else break from loop
            }
          }
        }
      }
  ...
  mutating func generate() -> Int {
    ...
    for row in (0..<cells.count) {
      for col in (0..<cells[0].count) {
        //print(row,col)
        let neighbours = self.neighbours(row: row, col: col)
        //print("checking",row,col)
        switch cell(row: row, col: col) {
        case "L": if neighbours == 0 { cellsCopy[row][col] = "#"; changed += 1 }
        case "#": if neighbours >= 4 { cellsCopy[row][col] = "L"; changed += 1 }
        default: break
        }
      }
    }
  ...
  mutating func generateLong() -> Int {
    var cellsCopy = cells
    var changed = 0
    for row in (0..<cells.count) {
      for col in (0..<cells[0].count) {
        //print(row,col)
        let neighbours = self.neighboursLong(row: row, col: col)
        //print("checking",row,col)
        switch cell(row: row, col: col) {
        case "L": if neighbours == 0 { cellsCopy[row][col] = "#"; changed += 1 }
        case "#": if neighbours >= 5 { cellsCopy[row][col] = "L"; changed += 1 } //Note 5 instead of 4
        default: break
        }
      }
    }
  ...
  func show() {
    for row in (0..<cells.count) {
      print(String(cells[row]))
    }
    print("=================")
  }
}

var grid = Grid(strings: lines)
var changes: Int = 0
grid.show()
...
//  print("[[[[[[[[[[[[[[[[[[[[[[[[[[[")
//  print(lastGrid,grid, grid == lastGrid)
//  print("]]]]]]]]]]]]]]]]]]]]]]]]]]]")
//
  changes = grid.generateLong() //For part A use generate, for part B use generateLong
  print(changes)
} while changes > 0

print(grid.occupiedSeats())
