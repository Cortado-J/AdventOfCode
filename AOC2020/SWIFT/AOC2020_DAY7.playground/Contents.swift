...
struct Bag {
  var colour: String
  var contents: [(number: Int, colour: String)]
}
...
for line in lines {
  ...
  let bagInsideCount = (words.count / 4) - 1
  for bagInside in (0..<bagInsideCount) {
    let offset = 4 + 4*bagInside
    bag.contents.append( (Int(words[offset])!, words[offset+1]+" "+words[offset+2]) )
  }
  bags.append(bag)
}

func findHoldersOfShiny() {
  var holdersOfShiny: Set<String> = ["shiny gold"]
  
  var lastCount = 0
  
  repeat {
    lastCount = holdersOfShiny.count
    
    for bag in bags {
      for holder in holdersOfShiny {
        if bag.contents.map({$0.1}).contains(holder) {
          holdersOfShiny.insert(bag.colour)
        }
      }
    }
  } while holdersOfShiny.count > lastCount // Keeep going intil no more!
  
  print(holdersOfShiny)
  print(holdersOfShiny.count-1)
}

func findBagsInShiny() {
  var bagLookup: [String:Bag] = [:]
  for bag in bags {
    bagLookup[bag.colour] = bag
  }

  func bagsIn(content: (Int, String)) -> Int {
    content.0 * (1 + bagsIn(colour: content.1))
  }

  func bagsIn(colour: String) -> Int {
    print("Finding bags in `\(colour)`")
    return bagLookup[colour]!
      .contents
      .reduce(0){
        $0 + bagsIn(content: $1)
    }
  ...
  print(bagsIn(colour: "shiny gold"))
}
