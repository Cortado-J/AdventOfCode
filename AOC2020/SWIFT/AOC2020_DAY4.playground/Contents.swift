...
let target: Set<String> = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"]  // "cid" optional
...
func check(_ found: Dictionary<String, String>) -> Bool {
  //Now check individual field values:
  //byr (Birth Year) - four digits; at least 1920 and at most 2002.
  guard let byr: Int = Int(found["byr"]!) else { return false }
  guard byr >= 1920 && byr <= 2002 else { return false }
  
  //iyr (Issue Year) - four digits; at least 2010 and at most 2020.
  guard let iyr: Int = Int(found["iyr"]!) else { return false }
  guard iyr >= 2010 && iyr <= 2020 else { return false }

  //eyr (Expiration Year) - four digits; at least 2020 and at most 2030.
  guard let eyr: Int = Int(found["eyr"]!) else { return false }
  ...
  guard let height = Int(found["hgt"]!.dropLast(2)) else { return false }
  switch units {
  case "cm": guard height >= 150 && height <= 193 else { return false }
  case "in": guard height >=  59 && height <=  76 else { return false }
  ...
  for (index, char) in hcl.enumerated() {
    if index == 0 {
      guard char == "#" else { return false }
    } else {
      guard "0123456789abcdef".contains(char) else { return false }
    }
  }
    
  //ecl (Eye Color) - exactly one of: amb blu brn gry grn hzl oth.
  let ecl = found["ecl"]!
  guard ecl.count == 3 else { return false }
  guard ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].contains(ecl) else { return false }

  //pid (Passport ID) - a nine-digit number, including leading zeroes.
  let pid = found["pid"]!
  guard pid.count == 9 else { return false }
  for (index, char) in pid.enumerated() {
    guard "0123456789".contains(char) else { return false }
  }
...
func process() {
  if gather.count > 0 {
    ...
    for word in gather {
      let name = String(word.prefix(3))
      let value = String(word.dropFirst(4))
      fields[name] = value
    }
    let allFieldsFound = target.subtracting(fields.keys).isEmpty
    print(fields, target, allFieldsFound)
    if allFieldsFound {
      if check(fields) {
        count += 1
      }
...
for line in lines {
  ...
  gather.append(contentsOf: words)
}
process()
